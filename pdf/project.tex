...
\abstract{
  ...
  used Monte-Carlo methods matches theoretical expectations.
...
\subsection{Quadrature Methods}
  ...
  \begin{equation}
  \int_a^b f(x) dx =
  \int_a^b f(a) + \frac{f(b) - f(a)}{b-a} (x-a) + \mathcal{O}(x^2) dx =
  ...
  \footnotetext{Note that on should take a number of steps before using this estimate. \cite{nr} recommend
  $5$ steps.}
